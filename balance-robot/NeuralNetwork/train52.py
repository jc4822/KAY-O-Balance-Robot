...
class DepthDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print(base_name,KRKiUV_Ts, KT_Ts)
        return left_tensor, right_tensor, depth_tensor, KRKiUV_Ts, KT_Ts
...
def train():
    ...
    # 初始化模型
    model = DepthNet52(height=144, width=192).to(device)
    
    # 加载预训练权重（如果存在）
    model_path = "depth_model_52.pth"
    ...
    for epoch in range(epochs):
        ...
        for left_imgs, right_imgs, depths, KRKiUV_Ts, KT_Ts in loader:
            # 移动数据到设备
            ...
            # 前向传播
            optimizer.zero_grad()
            refined, out1, out2, out3 = model(left_imgs, right_imgs, KRKiUV_Ts, KT_Ts)
            ...
            # 总损失（加权求和）
            total_batch_loss = loss_refined + 0.5 * (loss_out1 + loss_out2 + loss_out3)
    ...
    with torch.no_grad():
        for j in range(100):
            left_img, right_img, depth, KRKiUV_T, KT_T = test_dataset[j]
            ...
            #KRKiUV_T = KRKiUV_T.unsqueeze(0).to(device)
            #KT_T = KT_T.unsqueeze(0).to(device)
            #print(left_img.shape)
            #print(left_img.shape)
            ...
            for i in range(1):
                ...
                #print(KRKiUV_T[i].shape)
                #print(KT_T[i].shape)
            # 预测深度
            refined_depth, _, _, _ = model(left_img, right_img, KRKiUV_T, KT_T)
