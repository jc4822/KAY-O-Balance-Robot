...
float MPU_GYRO_Y_OFFSET = -0.068354;
...
float movement_speed_request = 0.0f;
float yaw_effort_request = 0.0f;

const float ROBOT_FORWARD_SPEED  = 1.0f * DEG_TO_RAD;
const float ROBOT_BACKWARD_SPEED = -1.0f * DEG_TO_RAD;
const float ROBOT_YAW_RATE       = 1.0f;
...
void setup() {
    ...
    const int numReadingsForCalibration = 300;
    const int targetReadingIndex        = 300;
    float tempAngleSum = 0.0f;
    ...
    movement_speed_request = 0.0f;
    yaw_effort_request     = 0.0f;
}
...
void loop() {
    ...
    if (millis() >= printTimer) {
        ...
        Serial.print(" | PID_Out: "); Serial.print(pid_output, 2);
        Serial.print(" | Mv_Req: "); Serial.print(movement_speed_request, 2);
        Serial.print(" | Yaw_Req: "); Serial.print(yaw_effort_request, 2);
        Serial.print(" | M1_Spd: "); Serial.print(step1.getSpeedRad(), 2);
        Serial.print(" | M2_Spd: "); Serial.print(step2.getSpeedRad(), 2);
        Serial.println();
    }
}
...
void handleSerialInput() {
    if (Serial.available() > 0) {
        ...
        if (cmdStr == "W") {
            ...
            yaw_effort_request   =  ROBOT_YAW_RATE;
            angle_setpoint       = 0.0f;
...
void applyMotorSpeeds() {
    float motor1_target_speed_rad_s = pid_output - yaw_effort_request;
    float motor2_target_speed_rad_s = pid_output + yaw_effort_request;
...
void performGyroCalibration() {
    ...
    if (!mpu.begin()) {
        Serial.println("MPU6050 not found for calibration!");
        return;
    }
    mpu.setAccelerometerRange(MPU6050_RANGE_2_G);
    mpu.setGyroRange(MPU6050_RANGE_250_DEG);
